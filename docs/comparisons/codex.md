...
`claude --continue` resumes your last session exactly where you left off. `--resume` lets you pick any past session. combined with the lore plugin, you get a searchable sqlite database of every session, every tool call, every file touched. codex has session history but no equivalent mining/analysis layer.
