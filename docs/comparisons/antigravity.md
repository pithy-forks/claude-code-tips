...
|---------|------------|-------------------|
| pricing | free tier / $20 pro / $100 max 5x / $200 max 20x per month | **verify current status at [antigravity.google/pricing](https://antigravity.google/pricing)** -- public preview ended; pricing structure subject to change |
...
every claude code session produces parseable JSON transcripts. the lore plugin builds sqlite databases from session data -- searchable by file, tool, cost, model, duration. antigravity has conversation history in its UI but no export or analysis layer.
