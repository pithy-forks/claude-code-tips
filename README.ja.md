...
- **`lore@cc`** セッションごとに sqlite に採掘して記録。コスト、ツール、エラー、ホットスポット、ループを問い合わせたり、自分の履歴全体を全文検索できる。すべてローカル。
...
/plugin install lore@cc                             # lore をインストール（セッション分析）
...
**[lore](./plugins/lore/)**を手に入れる・セッションマイニングを sqlite へ。コスト、検索、エラーメモリ、パターン検出。すべてのデータは`~/.claude/lore/lore.db`のローカルに残る。

```
/lore                     今日のセッション、コスト、よく使うツール
/lore search "websocket"  すべての会話を全文検索
/lore mistakes            Claude が繰り返すエラーパターン
/lore hotspots            セッション横断で最も編集されたファイル
/lore loops               セッション横断で繰り返されるパターン
```

`lore` + `safety-guard` フックから始めて、慣れたら追加していく。**[lore ドキュメント &rarr;](./plugins/lore/)**
...
/cc:time-calibrate              実際のスループット（lore.db から）をルールと比較
...
- **[lore](./plugins/lore/)** · セッションマイニングを sqlite へ。コスト、検索、エラーメモリ、パターン検出
