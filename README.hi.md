...
- **`lore@claude-code-tips`** हर session को sqlite में मined किया जाता है। costs, tools, errors, hotspots, loops और अपने पूरे history के across full-text search को query करो। सब कुछ local है।
- **`cc@claude-code-tips`** cross-session awareness और messaging। साथ ही एक `time` subsystem: `/cc:time-estimate` realistic claude-code time देता है जो तुम्हारे session history में grounded होता है, optimistic guesses नहीं।
- **`time@claude-code-tips`** 3-meter fuel gauge (5-hour session, 7-day weekly, 200k context)। pre-turn hook claude को cleaner handoffs की ओर nudge करता है जबकि meters भरते हैं। `/fuel state` उन्हें directly पढ़ता है; `/fuel handoff` एक stopping point को draft करता है।
