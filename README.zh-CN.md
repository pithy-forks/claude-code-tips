...
- **`lore@cc`** 每个会话都挖掘到 sqlite。查询成本、工具、错误、热点、循环，以及跨整个历史记录的全文搜索。完全本地化。
...
你获得 **[lore](./plugins/lore/)** · 会话挖掘到 sqlite。成本、搜索、错误记忆、模式检测。所有数据保存在本地 `~/.claude/lore/lore.db`。

```
/lore                     今天的会话、成本、热门工具
/lore search "websocket"  跨所有对话的全文搜索
/lore mistakes            Claude 重复犯的错误模式
/lore hotspots            跨会话中最常编辑的文件
/lore loops               跨会话重复的模式
```

从 `lore` 和 `safety-guard` hook 开始。随着进行添加更多。**[lore 文档 &rarr;](./plugins/lore/)**
...
/cc:time-calibrate              对照规则比较实际吞吐量（来自 lore.db）
