...
- **`lore@cc`** 每个会话都挖掘到 sqlite。查询成本、工具、错误、热点、循环，以及搜索你自己的历史记录。所有数据本地化。
- **`cc@cc`** 跨会话感知和消息传递。加上 `time` 子系统：`/cc:time-estimate` 基于你的会话历史给出现实的 claude-code 时间估计，而不是乐观的猜测。
- **`fuel@cc`** 3 档燃油表（5 小时会话、7 天周期、20 万 token 上下文）。当燃油表满时，PreToolUse hook 会推动 claude 进行更干净的切换。`/fuel state` 直接读取；`/fuel handoff` 草拟一个停止点。
...
/plugin install lore@cc                             # 安装 lore（会话分析）
